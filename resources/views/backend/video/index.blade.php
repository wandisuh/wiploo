...
<div class="right_col" role="main">
  <div class="">
	...
	<div class="row">
		...
		<div class="x_panel">
			<div class="col-md-1">
			  <a type="button" class="btn btn-primary" href="{{ route('add-video') }}">Tambah Video</a>
			</div>
		  <div class="x_title">
			<h2>&nbsp; <small>&nbsp;</small></h2>
			  ...
			  <tbody>
				@foreach($videos as $k=>$video)
					<tr>
					  ...
					  <td>Link Video</td>
					  <td>{{ str_limit($video->content,25) }}</td>
					  <td>
						<a href="{{ route('edit-video', ['id'=> $video->id]) }}" type="button" class="btn btn-success btn-sm">Edit</a>
						<button type="button" class="btn btn-sm btn-warning" onclick="sw_alert({{$video->id}})">Delete</button>
					  </td>
					</tr>
				@endforeach
			  </tbody>
...
<script type="text/javascript">
	function sw_alert(id) {
		...
		function(isConfirm){
		  if (isConfirm) {
			window.location = 'video/'+id+'/delete';
		  } else {
